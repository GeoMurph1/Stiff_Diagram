...
class StiffDiagram:
    ...
    def plot(self, style='reg', anion_color='orange', cation_color='blue', output='png', height=500, width=555):
        ...
        for l in locs[0:5]:
            ...
            p2 = figure(height=height, title=l, width=width, x_axis_label="meq/L",
                         y_range=FactorRange(factors=self.anion_axis_labels, range_padding = -0.3,
                                             range_padding_units = 'percent'), toolbar_location=None)
            #p2.title = l 
            #p2.y_range[0].bounds = (0, 2)
            p2.harea(x1=[0, 0, 0], x2=anion_data, y=self.anion_axis_labels, color=anion_color, alpha=0.8)
            p2.extra_y_ranges = {"cation_range": FactorRange(factors=self.cation_axis_labels,
                                                             range_padding = -0.3, range_padding_units = 'percent')}
            p2.add_layout(CategoricalAxis(y_range_name="cation_range"), 'right')
            p2.harea(x1=[0, 0, 0], x2=cation_data, y=self.anion_axis_labels, color=cation_color, alpha=0.8) # For this to work, cation y-axis must be same as anion y-axis; should be converted to single dummty axis to avoid confusion
